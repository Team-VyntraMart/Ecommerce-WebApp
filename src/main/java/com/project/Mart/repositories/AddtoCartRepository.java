...
@Repository
public interface AddtoCartRepository extends JpaRepository<AddtoCartModel,Long> {
	...
	Optional<AddtoCartModel> getCartByuserIdtest();
	@Query("Select addCart  FROM AddtoCart addCart WHERE addCart.product.id= :product_id and addCart.user_id=:user_id")
	Optional<AddtoCartModel> getCartByProductIdAnduserId(@Param("user_id")Long user_id,@Param("product_id")Long product_id);
